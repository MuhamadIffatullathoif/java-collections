package org.iffat.final_collections_challenge;
...
public class Cart {
    ...
    public void removeItem(InventoryItem item, int qty) {
        ...
        if (current <= qty) {
            ...
            products.merge(item.getProduct().sku(), qty, (oldValue, newVal) -> oldValue - newVal);
    ...
    public void printSalesSlip(Map<String, InventoryItem> inventory) {
        ...
        for (var cartItem : products.entrySet()) {
            ...
            total += itemizedPrice;
            System.out.printf("\t%s %-10 (%d)@ $%.2f = $%.2f%n", cartItem.getKey(),
